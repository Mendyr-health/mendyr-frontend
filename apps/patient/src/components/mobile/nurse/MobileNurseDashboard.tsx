    ...
    <div className="space-y-6 px-3 pt-2 pb-28">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-0.5 text-xs">Mobile Clinical Dashboard</p>
        </div>
      ...
      <motion.div
        ...
        className={`rounded-3xl p-5 ${config.bg} border-border flex items-center gap-4 border shadow-sm`}
      >
        <div className="shrink-0">{config.icon}</div>
        <div>
          <h2 className="text-foreground text-base font-bold">{config.label}</h2>
          <p className={`text-xs ${config.color} leading-relaxed font-medium`}>{config.desc}</p>
        </div>
      </motion.div>
