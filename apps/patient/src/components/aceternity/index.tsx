...
export function MovingBorder({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          'bg-hsl-background relative rounded-xl px-6 py-3',
          'bg-background',
          className,
        )}
      >
...
export function BentoGridItem({
  ...
  return (
    <motion.div
      className={cn(
        'group/bento row-span-1 rounded-xl transition duration-200 hover:shadow-xl',
        'bg-glass flex flex-col justify-between space-y-4 p-6',
        className,
      )}
...
const SPARKLES = Array.from({ length: SPARKLES_COUNT }, (_, index) => createSparkleConfig(index));
...
export function FloatingNavbar({
  ...
  return (
    <motion.nav
      ...
      className={cn(
        'fixed inset-x-0 top-6 z-50 mx-auto max-w-5xl',
        'bg-glass-strong rounded-full border-white/60 px-8 py-3 shadow-[0_8px_32px_rgba(5,17,242,0.12)]',
        className,
      )}
    >
      {children}
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="from-primary absolute top-0 h-48 bg-gradient-to-b to-transparent opacity-30 blur-3xl"
      />
      <motion.div
        initial={{ width: '16rem' }}
        ...
        className="from-primary-light absolute top-0 h-32 bg-gradient-to-b to-transparent opacity-20 blur-2xl"
      />
      {/* Lamp line */}
      <motion.div
        initial={{ width: '8rem' }}
        ...
        className="via-primary absolute top-0 h-[2px] bg-gradient-to-r from-transparent to-transparent"
      />
      <div className="relative z-10">{children}</div>
    </div>
  );
    ...
    >
      ...
      >
        {[...items, ...items].map((item, idx) => (
          <div
            key={idx}
            className="bg-glass text-foreground flex items-center gap-2 rounded-full px-6 py-3 text-sm whitespace-nowrap"
          >
            {item.icon}
            {item.text}
          </div>
