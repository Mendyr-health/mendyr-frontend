...
export function useAppointments() {
  ...
  const rejectedRequests = appointments.filter((apt) => apt.status === "REJECTED" || apt.status === "CANCELLED");
  ...
  const startVisit = (publicId: string) => {
    ...
    toast.info("Visit Started", {
      description: "You are now checked in. Don't forget to log clinical notes & vitals before checkout.",
    });
  };

  const completeVisit = (publicId: string, notesOrNote: string | CareNotePublic, vitals?: any, meds?: string[]) => {
    const noteObj: CareNotePublic = typeof notesOrNote === "string" ? {
      id: `note-${Date.now()}`,
