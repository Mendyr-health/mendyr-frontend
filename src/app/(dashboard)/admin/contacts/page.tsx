...
import { Search, Filter, ChevronLeft, ChevronRight, Eye, CheckCircle, MessageSquare } from "lucide-react";
...
export default function AdminContactsPage() {
  ...
  const fetchContacts = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) { setContacts(data.data || []); setTotalPages(data.meta?.totalPages || 1); }
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <div className="relative flex-1">
          ...
          <input type="text" value={query} onChange={(e) => { setQuery(e.target.value); setPage(1); }} placeholder="Search contacts..." className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-muted border border-border text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary" />
        </div>
        <select value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); setPage(1); }} className="px-4 py-2.5 rounded-xl bg-white/40 backdrop-blur-md border border-white/60 text-sm text-foreground focus:outline-none focus:bg-white/60 focus:ring-2 focus:ring-primary/40 shadow-sm transition-all">
          <option value="">All</option><option value="NEW">New</option><option value="READ">Read</option><option value="REPLIED">Replied</option><option value="ARCHIVED">Archived</option>
        </select>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead><tr className="border-b border-border">
                {["Name", "Subject", "Status", "Date", ""].map((h) => <th key={h} className="text-left px-4 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wider">{h}</th>)}
              </tr></thead>
              <tbody className="divide-y divide-white/5">
                {loading ? Array.from({ length: 5 }).map((_, i) => <tr key={i}><td colSpan={5} className="px-4 py-4"><div className="skeleton h-4 w-full" /></td></tr>) : contacts.length === 0 ? <tr><td colSpan={5} className="px-4 py-12 text-center text-sm text-muted-foreground">No inquiries found</td></tr> : contacts.map((c) => (
                  <motion.tr key={c.publicId} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={`hover:bg-muted cursor-pointer ${selectedContact?.publicId === c.publicId ? "bg-muted" : ""}`} onClick={() => setSelectedContact(c)}>
