    ...
    <div className="space-y-8 pt-8 lg:pt-0">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-2xl md:text-3xl font-bold text-foreground font-outfit">
          Welcome, <span className="text-primary">{user?.fullName?.split(" ")[0] || "Nurse"}</span>
        </h1>
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            <h2 className="text-lg font-semibold text-foreground">
              Application Status: <span className={`text-${config.color}-500`}>{config.label}</span>
            </h2>
        ...
        <div className="relative">
          ...
          {[
            ...
            <div key={idx} className="relative flex items-center gap-4 pb-6 last:pb-0">
              <div className={`relative z-10 h-10 w-10 rounded-full flex items-center justify-center ${step.done ? "bg-primary/20 text-primary" : "bg-muted text-muted-foreground"}`}>
                {step.done ? <CheckCircle className="h-5 w-5" /> : <div className="h-3 w-3 rounded-full bg-muted-foreground/30" />}
              </div>
