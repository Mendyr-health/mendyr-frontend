    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading ? (
          ...
          <p className="col-span-full text-center text-sm text-muted-foreground py-12">No services found</p>
        ) : (
          services.map((s, idx) => (
            <motion.div key={s.publicId} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }} className="bg-glass rounded-xl p-5">
              <div className="flex items-start justify-between mb-3">
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full ${s.isActive ? "text-success bg-success/10" : "text-muted-foreground bg-muted"}`}>
